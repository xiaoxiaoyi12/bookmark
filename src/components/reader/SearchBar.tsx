...
export default function SearchBar({ bookId, readerRef }: Props) {
  ...
  return (
    <div className="absolute top-12 right-4 z-40 w-96 bg-gray-800/95 backdrop-blur-sm rounded-xl shadow-2xl border border-gray-600/50 overflow-hidden select-none">
      {/* 搜索输入 */}
      <div className="flex items-center gap-2 p-3 border-b border-gray-700/60">
        ...
        />
        {searching && (
          <span className="text-[10px] text-gray-500">搜索中...</span>
        )}
        {!searching && searchQuery && (
          <span className="text-[10px] text-gray-500">{totalCount} 条结果</span>
        )}
        <div className="flex gap-0.5">
          <button onClick={handlePrev} className="p-1 text-gray-400 hover:text-white rounded hover:bg-gray-700/60" title="上一个">
            <svg className="w-3 h-3" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 10L5 6l4-4" /></svg>
          </button>
          <button onClick={handleNext} className="p-1 text-gray-400 hover:text-white rounded hover:bg-gray-700/60" title="下一个">
            <svg className="w-3 h-3" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 2l4 4-4 4" /></svg>
          </button>
        </div>
        <button onClick={toggleSearch} className="p-1 text-gray-400 hover:text-white rounded hover:bg-gray-700/60" title="关闭">
      ...
      {/* Tabs */}
      {searchQuery && (
        <div className="flex gap-1 px-3 py-2 border-b border-gray-700/40">
          ...
          {currentResults.length === 0 ? (
            ...
            currentResults.map((r, i) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 text-xs border-b border-gray-700/30 transition-colors ${
                  i === currentIdx ? 'bg-blue-600/20' : 'hover:bg-gray-700/40'
                }`}
              >
                <div className="flex items-start gap-2">
                  {r.page && (
                    <span className="shrink-0 px-1.5 py-0.5 bg-gray-700 rounded text-[10px] text-gray-400">
                      P{r.page}
                    </span>
                  )}
                  <span className="text-gray-300 line-clamp-2 leading-relaxed">
